  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button variant="outline" onClick={() => router.push("/cars/new")} disabled={loading}>
          手入力へ
        </Button>
      </div>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="space-y-1">
            ...
            {statuses.length > 0 ? (
              ...
              <Input value={status} onChange={(e) => setStatus(e.target.value)} placeholder="例: 仕入済 / 整備中 / 販売中" />
            )}
          </div>
          ...
          <div className="space-y-1">
            <Label>年式</Label>
            <Input inputMode="numeric" value={year} onChange={(e) => setYear(e.target.value)} placeholder="例: 2020" />
          </div>

          <div className="space-y-1">
            <Label>登録番号</Label>
            <Input value={carNumber} onChange={(e) => setCarNumber(e.target.value)} placeholder="例: 品川 300 あ 12-34" />
          </div>

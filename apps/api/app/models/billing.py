...
class BillingDocumentORM(Base):
    ...
    kind = Column(String(32), nullable=False, index=True)    # estimate / invoice
    status = Column(String(32), nullable=False, index=True)  # draft / issued / void

    doc_no = Column(String(32), nullable=True, index=True)
    ...
    __table_args__ = (
        UniqueConstraint("store_id", "doc_no", name="uq_billing_documents_store_doc_no"),
        Index("ix_billing_documents_store_kind_status", "store_id", "kind", "status"),
    )
...
class BillingLineORM(Base):
    ...
    # BillingDocumentORM.lines と対になる
